...
export function Experience() {
  const [begun, setBegun] = useState(false); const [warm, setWarm] = useState(false); const [revealed, setRevealed] = useState(false); const reduced = useReducedMotionPreference();
  ...
  return <main className={`experience ${begun ? "has-begun" : ""}`}><Atmosphere warm={warm || revealed}/><motion.div className="progress" style={{ scaleX: progress }}/><a className="skip-link" href="#story">Skip introduction</a>
    ...
    <div id="story" className="story" aria-hidden={!begun}>
      ...
      {site.chapters.slice(0, 2).map((chapter) => <Chapter key={chapter.number} chapter={chapter}/>)}
      <MusicMoment onPlay={() => setWarm(true)}/>
      {site.chapters.slice(3).map((chapter) => <Chapter key={chapter.number} chapter={chapter}/>)}
      <Letter/>
