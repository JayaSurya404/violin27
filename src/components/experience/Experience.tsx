...
export function Experience() {
  ...
  return <main className={`experience ${begun ? "has-begun" : ""}`}><Atmosphere warm={revealed}/><motion.div className="progress" style={{ scaleX: progress }}/><a className="skip-link" href="#story">Skip introduction</a>
    ...
    <div id="story" className="story" aria-hidden={!begun}>
      <section className="story-section threshold"><div className="chapter-mark">01 / before I say it</div><Flower/><h1>{site.openingTitle}</h1><p>{site.intro}</p><div className="down-line"/></section>
      ...
      <footer className="ending"><Flower bloom/><p>{site.final.first}</p><p>{site.final.second}</p><h2>I love you, <em>{site.person.split(" ")[0]}.</em></h2><div className="signature"><strong>{site.creator}</strong></div><small>That’s all I wanted to say.</small></footer>
    </div>
  </main>;
}
